...
| `bd` | border width from `vars.size.line` |
| `bdl`, `bdr`, `bdt`, `bdb` | side-specific border widths from `vars.size.line` |
| `blc`, `brc`, `btc`, `bbc` | side-specific border colors from the SUIS color map |
| `r` | border radius for all corners |
| `tlr`, `trr`, `blr`, `brr` | corner-specific radius values |
| `text` | semantic font style from `vars.font` |
| `shadow` | semantic shadow from `vars.shadow` |
| `overflow` | `auto`, `hidden`, `visible`, `scroll`, or axis-specific values such as `xAuto` and `yHidden` |
